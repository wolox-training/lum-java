...
public class UserService {
    ...
    public Users createUser(Users users) {
        log.info(String.format("%s was received to POST", users.toString()));
        return userRepository.save(users);
    }

    public Users readUser(long id) {
        log.info(String.format("%d was received to search Users",id));
        return userRepository.findById(id).orElseThrow(UserNotFoundException::new);
    }

    public Users updateUsers(Users users, long id) {
        log.info(String.format("%s (users) and %d (id) was received to look for User",users,id));
        if (users.getId() != id ) {
    ...
    public void deleteUser(long id) {
        log.info(String.format("%d was received to delete user", id));
        readUser(id);
        log.info("User was found");
    ...
    public void removeBook(Book book, long id) {
        log.info(String.format("%s (book) and %d (id) was received to remove book to user",book,id));
        Users user = readUser(id);
        if (bookAlreadyExists(user, book)) {
            user.removeBook(book);
            updateUsers(user, id);
            log.info("Book was removed");
        } else {
